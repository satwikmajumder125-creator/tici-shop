...
// Resolve image stored by admin to a full URL usable in frontend
function imgUrl(?string $path): string {
    if (!$path) return '';
    if (str_starts_with($path, 'http')) return $path;
    $path = ltrim($path, '/');
    if (str_starts_with($path, 'uploads/')) return 'http://localhost/tici-admin/' . $path;
    if (str_starts_with($path, 'tici-admin/')) return 'http://localhost/' . $path;
    return $path;
}
