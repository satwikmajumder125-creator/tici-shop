...
// Note: status check uses IN (1,'1','active') for broad compatibility
foreach ($categories as &$cat) {
    $cid = (int)$cat['id'];

    // product count — check status=1 OR status='active'
    $cnt = $db->prepare("SELECT COUNT(*) FROM products WHERE category_id = ? AND (status = 1 OR status = 'active')");
    $cnt->execute([$cid]);
    $cat['product_count'] = (int)$cnt->fetchColumn();

    // min price
    $mp = $db->prepare("SELECT MIN(COALESCE(sale_price, price)) FROM products WHERE category_id = ? AND (status = 1 OR status = 'active')");
    $mp->execute([$cid]);
    $cat['min_price'] = $mp->fetchColumn() ?: null;

    // sample image — first primary image of best product
    $si = $db->prepare("
        SELECT IFNULL(pi.image_path, IFNULL(pi.image, '')) AS img
        FROM products p
        LEFT JOIN product_images pi ON pi.product_id = p.id AND pi.is_primary = 1
        WHERE p.category_id = ? AND (p.status = 1 OR p.status = 'active')
        ORDER BY p.featured DESC, p.id DESC
        LIMIT 1
    ");
    $si->execute([$cid]);
    $row = $si->fetch(PDO::FETCH_ASSOC);
    $cat['sample_img'] = ($row && !empty($row['img'])) ? $row['img'] : '';
}
unset($cat);

// Total product count for "All Products" card
$totalProducts = (int)$db->query("SELECT COUNT(*) FROM products WHERE status = 1 OR status = 'active'")->fetchColumn();
  ...
  <div class="container">
    <div class="cats-grid">
      ...
      <a href="<?= SITE_URL ?>/shop/index.php" class="cat-list-card">
        <div class="clc-img">
          <span>🛍️</span>
          <div class="clc-count"><?= $totalProducts ?> product<?= $totalProducts!==1?'s':'' ?></div>
        </div>
        <div class="clc-body">
      ...
      <a href="<?= SITE_URL ?>/shop/categories.php?category=<?= h($cat['slug']) ?>"
        ...
        <div class="clc-img">
          ...
          <?php else: ?>
            <span><?= $emoji ?></span>
          <?php endif; ?>
          <?php $pcount = (int)($cat['product_count'] ?? 0); if ($pcount > 0): ?>
          <div class="clc-count"><?= $pcount ?> product<?= $pcount!==1?'s':'' ?></div>
          <?php endif; ?>
        </div>
        <div class="clc-body">
          <div class="clc-name"><?= h($cat['name']) ?></div>
          <?php $mp = $cat['min_price'] ?? null; if ($mp): ?>
          <div class="clc-from">From <strong><?= price((float)$mp) ?></strong></div>
